...
// Instantiation test
let digit = new Digit()
assert.equal(digit.print(), 'A', 'A instantiate digit should start with value A')
console.log(digit.print())

// Testing 25 increments (all letters)
digit = new Digit()
incrementDigitBy(digit, 25)
assert.equal(digit.print(), 'Z', 'If next() was called 25x, value of the digit should be C')
console.log(digit.print())

// Testing 26 increments (first value with 2 letters)
digit = new Digit()
incrementDigitBy(digit, 26)
assert.equal(digit.print(), 'AA', 'If next() was called 26x, value of the digit should be AA')
console.log(digit.print())

digit = new Digit()
incrementDigitBy(digit, Math.pow(26, 1) - 1) // max value with one char
assert.equal(digit.print(), 'Z', 'If next() was called 51x, value of the digit should be AA')
console.log(digit.print())

digit = new Digit()
incrementDigitBy(digit, Math.pow(26, 1) + Math.pow(26, 2) - 1)
assert.equal(digit.print(), 'ZZ', 'Max value with two chars should be ZZ')
console.log(digit.print())

digit = new Digit()
let increments = getIncrementsForMaxValue(5) - 1
incrementDigitBy(digit, increments)
assert.equal(digit.print(), 'ZZZZZ', `After ${increments} increments, which is the max value with 5 chars, value should be ZZZZZ`)
console.log(digit.print())

digit = new Digit()
incrementDigitBy(digit, getIncrementsForMaxValue(5))
increments = getIncrementsForMaxValue(5)
assert.equal(digit.print(), 'AAAAAA', `After ${increments} increments, which is the first value with 6 chars, value should be AAAAAA`)
console.log(digit.print())
